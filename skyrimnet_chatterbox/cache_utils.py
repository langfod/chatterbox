...
def _move_conditionals_to_device_dtype(cond_cls, device, dtype):
    ...
    CRITICAL: All floating point tensors must have matching dtype or T3 inference
    will be extremely slow due to dtype casting during computation.
    """
    try:
        # Move the entire conditionals object to device first
        cond_cls = cond_cls.to(device)
        
        # Ensure T3 conditionals have correct dtype for ALL float tensors
        if hasattr(cond_cls, 't3') and cond_cls.t3 is not None:
            # Use T3Cond's .to() method which handles dtype properly
            cond_cls.t3 = cond_cls.t3.to(device=device, dtype=dtype)
        
        # Ensure S3Gen conditionals are on correct device (these stay float32 typically)
...
def load_conditionals_cache(language: str="en", cache_key: str=None, model=None, device=None, dtype=None, enable_memory_cache=True, enable_disk_cache=True):
    ...
    try:
        ...
        if enable_memory_cache:
            ...
            if cached is not None:
                # Ensure correct device and dtype for ALL tensors (not just speaker_emb)
                # This is critical for performance - wrong dtype causes massive slowdowns
