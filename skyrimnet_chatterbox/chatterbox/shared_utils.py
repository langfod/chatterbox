...
def check_exaggeration_update_needed(
    ...
    needs_update = not torch.allclose(
        current_emotion_adv, new_emotion_tensor, atol=atol)
...
def validate_language_id(language_id: str, supported_languages: dict) -> str:
    ...
    if language_id is None:
        raise ValueError("language_id is required for multilingual TTS. Use one of: " +
                         ", ".join(supported_languages.keys()))

    if not isinstance(language_id, str):
        raise TypeError(
            f"language_id must be a string, got {type(language_id)}")
...
def validate_float_parameter(value, name: str, min_val: float = None, max_val: float = None, allow_zero: bool = True) -> float:
    ...
    if min_val is not None:
        if not allow_zero and value <= min_val:
            raise ValueError(
                f"{name} of {value} must be greater than {min_val}")
...
def validate_audio_prompt_path(audio_prompt_path: Union[str, Path]) -> Path:
    ...
    if not audio_path.exists():
        raise FileNotFoundError(
            f"Audio prompt file not found: {audio_prompt_path}")

    if not audio_path.is_file():
        raise ValueError(
            f"Audio prompt path is not a file: {audio_prompt_path}")

    # Check file extension
    valid_extensions = {'.wav', '.mp3', '.flac', '.m4a', '.aac', '.ogg'}
    if audio_path.suffix.lower() not in valid_extensions:
        raise ValueError(
            f"Unsupported audio format: {audio_path.suffix}. Supported: {', '.join(valid_extensions)}")
...
def estimate_token_count(text: str, tokenizer, language_id: str = None) -> int:
    ...
    try:
        if hasattr(tokenizer, 'encode'):
            ...
            #print(f"Token count for text: {len(tokens)}")
            return len(tokens)
        else:
    ...
    for sentence in sentences:
        ...
        if tokenizer:
            token_count = estimate_token_count(
                test_chunk, tokenizer, language_id)
            #print(f"Token count for test chunk: {token_count} (max_tokens: {max_tokens})")
        ...
        if tokenizer:
            ...
            if token_count > max_tokens:
                ...
                for i, quote in enumerate(chunk_quotes):
                    ...
                    chunk_with_placeholders = chunk_with_placeholders.replace(
                        quote, placeholder, 1)
                ...
                for word in words:
                    ...
                    for i, quote in enumerate(chunk_quotes):
                        placeholder = f"__CHUNKQUOTE_{i}__"
                        test_restored = test_restored.replace(
                            placeholder, quote)

                    if tokenizer:
                        test_tokens = estimate_token_count(
                            test_restored, tokenizer, language_id)
...
def _get_cached_silence_tensor(silence_samples: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    if cache_key not in _silence_cache:
        _silence_cache[cache_key] = torch.zeros(
            silence_samples, device=device, dtype=dtype)
...
def concatenate_audio_tensors(audio_tensors: List[torch.Tensor], silence_duration: float = 0.1, sample_rate: int = S3GEN_SR) -> torch.Tensor:
    ...
    for i, audio in enumerate(audio_tensors):
        ...
        if silence is None:
            ...
            silence = _get_cached_silence_tensor(
                silence_samples, audio.device, audio.dtype)
