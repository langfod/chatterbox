...
class CausalConditionalCFM(ConditionalCFM):
    ...
    def basic_euler(self, x, t_span, mu, mask, spks, cond, disable_tqdm=True):
        in_dtype = x.dtype
        x, t_span, mu, mask, spks, cond = cast_all(x, t_span, mu, mask, spks, cond, dtype=self.estimator.dtype)

        for t, r in tqdm(zip(t_span[..., :-1], t_span[..., 1:]), total=t_span.shape[-1] - 1, desc="S3 Token -> Mel", disable=disable_tqdm):
            t, r = t[None], r[None]
